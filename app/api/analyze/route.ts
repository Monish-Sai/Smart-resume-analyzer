...
${trimmedText}`;
    ...
    if (!apiKey || apiKey.includes("your_huggingface_api_key_here")) {
      // Mock Response for Demonstration
      const mockScore = Math.floor(Math.random() * 20) + 75; 
      const mockOutput = `Match Percentage: ${mockScore}
Score Breakdown: Skills matched the job description well, but experience level was slightly below requirements.
Matched Skills: JavaScript, React, Problem Solving
Missing Skills: Advanced CI/CD, specific domain knowledge
Suggestions:
- Add more quantifiable metrics to your recent role.
- Detail your experience with deployment pipelines.`;

      return NextResponse.json({ 
        result: mockOutput,
        structured: {
          score: mockScore,
          breakdown: "Skills matched the job description well, but experience level was slightly below requirements.",
          strengths: "JavaScript, React, Problem Solving",
          missing: "Advanced CI/CD, specific domain knowledge",
          suggestions: "- Add more quantifiable metrics to your recent role.\n- Detail your experience with deployment pipelines."
        }
      });
    }
