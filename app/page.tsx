...
import { useAuth, SignUpButton, SignInButton, useSession } from "@clerk/nextjs";
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      if (data.structured) {
        ...
        console.warn("Structured data missing. Raw output:", output);
      ...
      if (userId && session) {
        ...
        const supabaseWithAuth = supabaseAccessToken ? createClerkSupabaseClient(supabaseAccessToken) : supabase;
        ...
        if (uploadError) {
          console.warn("Cloud Synchronization Failed:", uploadError);
        }
      ...
      {isSignedIn && (
        <div className="w-full flex items-center flex-col animate-fadeIn relative">
          ...
          <motion.div layout className="w-full max-w-2xl bg-white dark:bg-zinc-900 border border-zinc-200/50 dark:border-zinc-800/50 rounded-3xl p-6 md:p-8 space-y-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.1)] transition-all duration-500 hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] dark:hover:border-zinc-700/50">
            ...
            <div
              {...getRootProps()}
              className={`relative group p-6 md:p-10 rounded-2xl border-2 border-dashed transition-all duration-300 flex flex-col items-center justify-center cursor-pointer ${isDragActive ? 'border-blue-500 bg-blue-50 dark:bg-blue-500/10' : 'border-zinc-300 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-900 hover:border-blue-400 dark:hover:border-blue-500/50'
                }`}
            >
              <input {...getInputProps()} />
            ...
            <button
              ...
              disabled={loading}
              className={`w-full py-4 rounded-xl font-medium transition-all duration-300 shadow-sm flex items-center justify-center gap-2 ${loading
